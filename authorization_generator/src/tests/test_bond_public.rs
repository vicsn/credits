...
/// Samples the execution for `bond_public`.
pub(crate) fn sample_bond_public(rng: &mut TestRng) -> Result<Transaction<CurrentNetwork>> {
    // Sample the sender.
    let (private_key, _) = sample_account(rng);
    // Read the request from the file.
    let request_string = std::fs::read_to_string("../bond_public_request.txt")?;
